...
void appSdvPbDeleteOsdShow(void)
{
	appLcmOsdClean();
	
	//Mode Icon	
	appLcmOsdLargeIconShow(LcmOsdIconIndex0, ID_ICON_SCDV_DELETE_MODE, FALSE);
...
void appSdvPbDeleteStateInit(UINT32 preState)
{
	UINT32 mode;
...
void appSdvPbDeleteStateOnMsg(UINT32 msg, UINT32 param)
{
	switch(msg)
	{
...
void appSdvPbDeleteStateOnKey(UINT32 msg, UINT32 param)
{
	switch(msg)
	{
...
void appSdvPbDeleteState(UINT32 msg, UINT32 param)
{
	switch (msg) 
	{
		case APP_STATE_MSG_INIT:
			RPrintf("appSdvPbDeleteState\n");
			appSdvPbDeleteStateInit(appPreviousStateGet());
			break;
		...
		default:
			...
			{
				appSdvPbDeleteStateOnKey(msg,param);
			}
			else
			{
				appSdvPbDeleteStateOnMsg(msg,param);
			}
			break;
	}
}
