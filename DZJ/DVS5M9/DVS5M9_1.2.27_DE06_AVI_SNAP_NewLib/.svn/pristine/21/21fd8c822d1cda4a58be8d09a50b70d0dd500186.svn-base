...
void appCdvVideoRecLapseDateStamp(void)
{
	...
	#else
	sprintf(	StampDateBuf,  "%d/%02d/%02d  %02d:%02d:%02d",	
			videoLapseRtc.tmx_mday, videoLapseRtc.tmx_mon,videoLapseRtc.tmx_year+1900,
			videoLapseRtc.tmx_hour, videoLapseRtc.tmx_min, videoLapseRtc.tmx_sec);
	#endif
...
static UINT8 nLapseAVIFileCreate=0;
UINT32 nImageHeigh=0;
UINT32 appCdvVideoRecLapsePreviewCallback(UINT32 fid, UINT8 *pbuf, UINT32 w, UINT32 h)
{
	//DBG_TIME_DECLARE(30);
	UINT32 starTime,endTime;
	
	//printf("--Lapse=%d w=%d h=%d ready=%d trigger=%d\n",fid,w,h,gVideoLapse.bYuvBufferReady,gVideoLapse.videoLapseTrigger);
	nImageHeigh = h;
	if(nLapseAVIFileCreate==1)
		return;	
	if(gVideoLapse.bYuvBufferReady == FALSE&&gVideoLapse.videoLapseTrigger==TRUE)
	{
		...
		//DBG_TIME_GET();
		//starTime = dbgTimeGet();
		//printf("--gVideoLapse.bYuvBufferReady=%d,\n", gVideoLapse.bYuvBufferReady);
...
UINT32 appCdvVideoRecLapseSwTrigCb(UINT8 *pbuf, sp5kYuvCbImgSize_t *pinfo, UINT32 rsvd0, UINT32 rsvd1)
{
	...
	printf("--trigger=%d yuvready=%d\n",gVideoLapse.videoLapseTrigger,gVideoLapse.bYuvBufferReady);
	...
	if(gVideoLapse.bYuvBufferReady == TRUE)
	{
		...
		__FUNC_TRACK__;
			...
			//for mosaic
			if(gVideoLapse.yuvBufferWidth==1920 && nImageHeigh==1072)
...
void appCdvVideoRecLapseStart(void)
{
	...
	//if(img_w!=gVideoLapse.yuvBufferWidth)
	{
	...
	//img_w = gVideoLapse.yuvBufferWidth;
	//img_h = gVideoLapse.yuvBufferHeight;
	
	sp5kModeSet(SP5K_MODE_VIDEO_PREVIEW);
	sp5kModeWait(SP5K_MODE_VIDEO_PREVIEW);
	nLapseAVIFileCreate=1;
	...
	///sp5kPreCallbackSet(15, appCdvVideoRecLapsePreviewCallback);
	//sp5kTimeDelay(SP5K_TIME_DELAY_1MS, 500);
	//sp5kPreviewCallbackSet(5, appCdvVideoRecLapsePreviewCallback, gVideoLapse.yuvBufferWidth, gVideoLapse.yuvBufferHeight);	
...
void appCdvVideoRecLapseStop(void)
{
	...
	UINT32 nWaitNum=0;
	appBtnDisable(BTN_ALL);//lapse
	sp5kPreCallbackSet(5, NULL);
	...
	appMsgFlush(APP_MSG_KEY_QUE);//lapse
	appBtnEnable(BTN_ALL);//lapse
...
void appCdvVideoRecLapseStateOnKey(UINT32 msg, UINT32 param)
{
	...
	switch(msg) 
	{
		...
		case APP_KEY_PRESS_OK:
			...
			{
				//strcpy(aviFile, appCdfsLastFileNameGet());
				if(gVideoCB.CapSec<gVideoLapse.videoLapseInterval)
					nLapseNoFrameAction=1;
