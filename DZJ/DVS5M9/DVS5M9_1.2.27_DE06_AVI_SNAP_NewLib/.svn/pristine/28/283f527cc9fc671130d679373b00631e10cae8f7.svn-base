...
{
	UINT32 msgId=0, param=0;/*message parameter*/
	UINT8 verStr[32];/*ver=version*/
	...
	while ( 1 ) 
	{
			...
			evbSimpleJobDo(msgId, param);/*evb=???*/
		#else	
				#if MSG_NEW_PROC/*PROC=procedure*/
	...
	switch( msgId )
	{
		...
		case SP5K_MSG_AE_READY:
		{
			...
			else 
			{
				...
				/***TODO****evinfo != evsync???*/
				expidx = aeResult.evInfo.expIdx;/*info=information*/
