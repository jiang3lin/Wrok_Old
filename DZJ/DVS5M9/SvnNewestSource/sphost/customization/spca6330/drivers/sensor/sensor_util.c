...
)
{
	...
	pbuf = hostAfeCmdFileRead(fname, NULL, 0, &fsize);

	if (pbuf) {
		if (fsize) {
			err = hostAfeCmdBufExecute(pbuf, fsize, cmdportid, nbitaddr, nbitdata);
			if (err!=SUCCESS)
