...
#include "app_msg.h"
UINT8  SdvSlowWifiPreStatus=UI_SCDV_VIDEO_MODE_NORMAL;
/**************************************************************************
 *                           C O N S T A N T S                            *
 **************************************************************************/
...
void appSdvWifiModeOsdShow(void)
{
	appLcmOsdClean();
...
void appSdvWifiModeStateInit(UINT32 preState)
{
	UINT32 mode;
	...
	profLogAdd(0,"appSdvWifiModeStateInit s");
	...
	profLogAdd(0,"appSdvWifiModeStateInit e");
}

void appSdvWifiModeStateOnMsg(UINT32 msg, UINT32 param)
{
	switch(msg)
	{
...
extern UINT nWifiPhotoModeSave;
void appSdvWifiModeStateOnKey(UINT32 msg, UINT32 param)
{
	...
	switch(msg)
	{
		...
		case APP_KEY_PRESS_OK:
			if(appDiskStatusGet() == APP_DISK_STATUS_MOUNTED_OK)
			{
				...
				nWifiPhotoModeSave=puiPara->SdvPhotoMode;
				SdvSlowWifiPreStatus=puiPara->SdvVideoMode;
				nWifiRecKeySkip=FALSE;
				//view
				appBtnDisable(BTN_OK);
				appMsgFlush(APP_MSG_KEY_QUE);
				//printf("--wifi ok disable\n");
...
void appSdvWifiModeState(UINT32 msg, UINT32 param)
{
	switch (msg) 
	{
		case APP_STATE_MSG_INIT:
			RPrintf("appSdvWifiModeState\n");
			appSdvWifiModeStateInit(appPreviousStateGet());
			break;
		...
		default:
			...
			{
				appSdvWifiModeStateOnKey(msg,param);
			}
			else
			{
				appSdvWifiModeStateOnMsg(msg,param);
			}
			break;
	}
}
