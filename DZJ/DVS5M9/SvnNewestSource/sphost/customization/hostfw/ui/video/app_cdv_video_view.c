...
void CdvSeamlessSetting()
{
	...
	{
		case UI_SCDV_VIDEO_LOOP_TIME_2MIN:			
			sp5kMediaRecAttrSet(SP5K_MEDIA_ATTR_SEAMLESS_TIME_SLOT, 120); /* 120 Second */ 
			break;
		case UI_SCDV_VIDEO_LOOP_TIME_5MIN:
			
			sp5kMediaRecAttrSet(SP5K_MEDIA_ATTR_SEAMLESS_TIME_SLOT, 300); /* 300 Second */ 
			break;
		case UI_SCDV_VIDEO_LOOP_TIME_15MIN:
		default:			
			sp5kMediaRecAttrSet(SP5K_MEDIA_ATTR_SEAMLESS_TIME_SLOT, 900); /* 900 Second */ 
			break;
	}	
	sp5kMediaRecCfgSet(SP5K_MEDIA_REC_SEAMLESS_CALLBACK, (UINT32)appCdvVideoSeamlessCallback); 
...
void appCdvVideoPvStateInit(UINT32 preState)
{
	...
	CdvSeamlessSetting();
	if(nLapseNoFrameAction)
	{
		strcpy(aviFile, appCdfsLastFileNameGet());
		printf("--nLapseNoFrameAction=%s\n", aviFile);
		//appCdfsFileDel(aviFile);
		appCdfsTheNewestFileDel(CDFS_FILE_FOLDER_LAST);
		nLapseNoFrameAction=0;
