...
)
{
	...
	switch(msg){
		...
		case APP_KEY_PRESS_MENU:
			...
			{			
				ret = menuProcVidRecAlwaysParaExcute(layer, item, msg,puiPara->VideoSize);
			}
			else
			{
...
)
{
	...
	switch(msg){
		...
		case APP_KEY_PRESS_SET:
			if(mode == SP5K_MODE_VIDEO_RECORD)
			{			
				ret = menuProcVidRecAlwaysParaExcute(layer, item, msg,puiPara->VideoQuality);
			}
			else
			{
				menuProcParaSet(layer, item);
...
)
{
	...
	switch(msg){
		...
		case APP_KEY_PRESS_SET:
			appVideo_SetSeamLess(item);
			if(mode == SP5K_MODE_VIDEO_RECORD)
			{	
				ret = menuProcVidRecAlwaysParaExcute(layer, item, msg,puiPara->Seamless);
			}
			else
			{
				menuProcParaSet(layer, item);
...
)
{
	...
	switch(msg){
		case APP_KEY_PRESS_SET:
			if(mode == SP5K_MODE_VIDEO_RECORD)
			{			
				ret = menuProcVidRecAlwaysParaExcute(layer, item, msg,puiPara->VideoStampMode);
			}
			else
			{
				menuProcParaSet(layer,item);
...
)
{
	...
	switch(msg){
		...
		case APP_KEY_PRESS_RIGHT:
			...
			#if CAM_TYPE_CVR
			psoundcontrol->voicerec= puiPara->VoiceRec;
			#endif
			ret = MENU_IGNORE_DEFAULT;
			break;
		case APP_KEY_PRESS_UP:
...
)
{
	...
	sp5kModeGet(&mode);
	param = param; /* just for clearing waring */
	preVoiceRecSts = puiPara->VoiceRec;
	switch(msg){
		case APP_KEY_PRESS_UP:
		...
		case APP_KEY_PRESS_SET:
			...
			if((preVoiceRecSts == puiPara->VoiceRec)&&(mode == SP5K_MODE_VIDEO_RECORD))
			{
				ret = menuProcVidRecAlwaysVoiceRecExcute();
			}
			ret = MENU_IGNORE_DEFAULT;
			break;
		case APP_KEY_PRESS_MENU:
...
)
{
	...
	switch(msg){
		case APP_KEY_PRESS_SET:
			menuProcParaSet(layer, item);
			if(puiPara->PhotoBurst!=UI_PHOTOBURST_OFF){
...
)
{
	...
	switch (msg) {
		case APP_KEY_PRESS_SET:
			if((mode == SP5K_MODE_VIDEO_RECORD)&&(puiPara->VideoStampMode != UI_STAMP_OFF))
			{			
...
)
{
	...
	switch (msg) {
		case APP_KEY_PRESS_SET:
				menuProcParaSet(layer, item);
				CLEAR_OSD_SRC;
				appStateChange(APP_STATE_VIDEO_SYSTEM, STATE_PARAM_NORMAL_INIT);			
			break;
	}
...
)
{
	...
	layer = layer; /* just for clearing waring */
	para = para; /* just for clearing waring */
	item=item;
	msg=msg;
	...
	switch(msg)
	{
		case APP_KEY_PRESS_SET:
			if(item == FORMAT_YES)
			{
				if(mode == SP5K_MODE_VIDEO_RECORD)
				{
					printf("\033[35m card fromat in video rec\033[0m\n");
					ret = menuProcVidRecAlwaysFormatExcute();
				}
				else
				{
					if(_setupMenuFunctionFormat()) 
		        		{
			        		...
			        		{
			        			printf("\033[35m card fromat in playback\033[0m\n");
			        			appMenu_MenuExit(0);
							ret = MENU_IGNORE_DEFAULT;
			        		}
			        	}
