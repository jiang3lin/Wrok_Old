...
{
	#if KIT_LMI_CTRL
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_IRIS_P), 0);
...
void irisLargeActuate(void)
{
	...
	sp5kLmiTrig();
	#else
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_IRIS_P), 1);
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_IRIS_N), 0);	
	#endif
	#else
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_IRIS_P), 0);
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_IRIS_N), 1);	
	#endif
}
/*-------------------------------------------------------------------------
...
void mshutIdleActuate(	void)
{
	...
	#else
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_SHUTTER_N), 0);
	#endif
}
...
void mshutOpenActuate(void)
{
	...
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_SHUTTER_N), 1);
	#endif
}
...
void mshutCloseActuate(void)
{
	...
	sp5kLmiTrig();
	#else
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_SHUTTER_P), 0);
	_SP5K_IO_SET(SP5K_GPIO_GRP_LMI, GPIO_MASK(CFG_SHUTTER_N), 1);
	#endif
}


#endif /* IS_LENS_MOTOR_DRV_SH_IRIS */
