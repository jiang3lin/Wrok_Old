...
#define CAPTURE_EQUAL_PREVIEW       1
...
SINT32 cap_svidx_to_agcidx(SINT32 svidx)
{
	...
	/*--- 由於capture有minimum saturation gain的限制，
	 * 當要使用低於minimum sat. gain的ISO拍照時，必須以縮短曝光時間
	 * 以補償gain的限制。
	 */
...
{
	/*-- evdiff
	 * evdiff是用來記錄ASM Mode調整後的曝光
	 * 會與實際測光的結果相差多少EV。
	 * 實際上應該只有M-Mode會用告知使用者現在的設定會與
	 * AE測光結果差距多少，以便使用者據以補償。
...
static void _captureParaCalc(UINT8 iris,UINT8 iso,UINT8 shut)
{
	 ...
	 ********************************************/
	 ...
	 if(aeCb.aeset.aeb){		/*AEB*/
		...
		UINT32 exp_limit = EXP_1_08; /* night scene 最長曝光時間不超過1/8s */
